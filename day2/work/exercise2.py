...
# Cumulative donations to each candidate
...
def plot_contrib(subplot, contribs, ranges):
    ...
    for candidate in contribs.iterkeys():
        left_edges = n.arange(ranges[candidate][0],
                              ranges[candidate][1] + _spacing,
                              _spacing)

        cum_contrib = n.cumsum(contribs[candidate])
...
def plot_candidates(filename, ranges):
    ...
    with open(filename) as file:
        ...
        plot_contrib(subplot, contribs, ranges)
