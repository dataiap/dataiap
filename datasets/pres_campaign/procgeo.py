import csv,sys,datetime,collections,sqlite3,json
import itertools
...
data = {}
cur = db.execute('select state, sum(amount) from donations group by state')
data['ALL'] = dict(cur.fetchall())
...
for name in names:
    cur = db.execute('select state, sum(amount) from donations where name = ? group by state', (name,))
    data[name] = dict(cur.fetchall())
